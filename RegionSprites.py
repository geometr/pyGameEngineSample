...
class RegionSprites:
    ...
    def __init__(self):
        ...
        self.sprites[self.SHAVAR] = pygame.image.load('../assets/tile_shavar.png').convert_alpha()
        self.sizes[self.SHAVAR] = (20,10)
        self.sprites[self.SHAVAR_ALDAN] = pygame.image.load('../assets/tile_shavar_aldan.png').convert_alpha()
        self.sizes[self.SHAVAR_ALDAN] = (20,10)
        self.sprites[self.SHAVAR_GOO] = pygame.image.load('../assets/tile_shavar_goo.png').convert_alpha()
    ...
    def prepareSprites(self):
        for type, (width,height) in self.sizes.items():
            self.night_filter[type] = pygame.Surface((width,height)).convert_alpha()
            self.night_filter[type].fill((148,148,128,0))
            self.day_filter[type] = pygame.Surface((width,height)).convert_alpha()
            self.day_filter[type].fill((16,0,16,0))
            self.morning_filter[type] = pygame.Surface((width,height)).convert_alpha()
            self.morning_filter[type].fill((64,32,32,0))
            self.evening_filter[type] = pygame.Surface((width,height)).convert_alpha()
